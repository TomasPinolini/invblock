...
export async function GET() {
  ...
  const rateLimited = await checkRateLimit(user.id, "fci-types", RATE_LIMITS.securities);
  ...
  try {
    ...
    const types = await client.getFCITypes();
    ...
    console.error("[FCI Types] Error:", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to fetch fund types" },
      { status: 500 }
    );
  }
}
