...
interface PortfolioCardListProps {
  ...
  onToggleExpand: (row: PortfolioRow) => void;
        ...
        return (
          <div key={row.id}>
            <div
              ...
              className={cn(
                "card-elevated hover-lift px-4 py-3 cursor-pointer active:bg-zinc-800/60",
                isExpanded && "border-l-2 border-l-blue-500 rounded-b-none"
              )}
            >
            ...
            {/* Inline expansion below the card */}
            {isExpanded && (
              <div className="border border-t-0 border-zinc-800 rounded-b-xl bg-zinc-900/80 border-l-2 border-l-blue-500">
                <AssetExpandedRow
                  asset={row}
                  displayCurrency={displayCurrency}
                  onCollapse={() => onToggleExpand(row)}
                  onBuy={() => onBuy(row)}
                  onSell={() => onSell(row)}
                />
              </div>
